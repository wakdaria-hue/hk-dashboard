...
1. `compute_baselines()` - per hotel, from historical actuals only:
   - `minutes_per_room` = HK minutes worked / rooms to clean, both summed over
     the same trailing window. Team-wide by necessity: on most days several
     housekeepers work the same rooms with no record of who cleaned which, so
     there's no honest way to split it per person.
...
def compute_baselines(
    ...
    """Per-hotel baseline from historical actuals in a trailing window.
    ...
    occupancy store. `window_days=None` uses every matching historical day.
    """
    ...
    past_occupancy = occupancy_df[
        (occupancy_df["date"] >= window_start) & (occupancy_df["date"] <= window_end)
    ]

    for hotel in sorted(occupancy_df["hotel"].dropna().unique()):
